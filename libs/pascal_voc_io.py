...
class PascalVocWriter:
    ...
    def addBndBox(self, xmin, ymin, xmax, ymax, name, difficult,crew,fake,occluded,reflection,behindglass):
    ...
    def appendObjects(self, top):
        for each_object in self.boxlist:
            ...
            if int(float(each_object['ymax'])) == int(float(self.imgSize[0])) or (int(float(each_object['ymin']))== 1):
                truncated.text = "True" # max == height or min
            elif (int(float(each_object['xmax']))==int(float(self.imgSize[1]))) or (int(float(each_object['xmin']))== 1):
                truncated.text = "True" # max == width or min
            else:
                truncated.text = "False"
            difficult = SubElement(object_item, 'difficult')
            difficult.text = str( bool(each_object['difficult']) & True )
            
            crew = SubElement(object_item, 'crew')
            crew.text = str( bool(each_object['crew']) & True )
            
            fake = SubElement(object_item, 'fake')
            fake.text = str( bool(each_object['fake']) & True )
            
            occluded = SubElement(object_item, 'occluded')
            occluded.text = str( bool(each_object['occluded']) & True )
            
            
            
            reflection = SubElement(object_item, 'reflection')
            reflection.text = str( bool(each_object['reflection']) & True )

            behindglass = SubElement(object_item, 'behindglass')
            behindglass.text = str( bool(each_object['behindglass']) & True )
    ...
    def save(self, targetFile=None):
        ...
        my_xml=prettifyResult
        pp = pprint.PrettyPrinter(indent=4)
        #pp.pprint(json.dumps(xmltodict.parse(my_xml)))
        with open(targetFile+'.json', 'w') as out_file:
...
class PascalVocReader:
    ...
    def addShape(self, label, bndbox, difficult,crew,fake,occluded,reflection,behindglass):
        xmin = int(float(bndbox.find('xmin').text))
        ymin = int(float(bndbox.find('ymin').text))
        xmax = int(float(bndbox.find('xmax').text))
        ymax = int(float(bndbox.find('ymax').text))
        points = [(xmin, ymin), (xmax, ymin), (xmax, ymax), (xmin, ymax)]
        self.shapes.append((label, points, None, None, difficult,crew,fake,occluded,reflection,behindglass))
    ...
    def parseXML(self):
        ...
        for object_iter in xmltree.findall('object'):
            ...
            self.addShape(label, bndbox, crew)

            fake = False
            if object_iter.find('fake') is not None:
                crew = bool(int(object_iter.find('fake').text))
            self.addShape(label, bndbox, fake)
