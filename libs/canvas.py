...
class Canvas(QWidget):
    ...
    def mouseMoveEvent(self, ev):
        ...
        for shape in reversed([s for s in self.shapes if self.isVisible(s)]):
            ...
            index = shape.nearestCursor(pos, self.epsilon)
            if index is not None:
                if self.selectedVertex():
                    self.hShape.highlightClear()
                self.hVertex, self.hShape = 3 - index, shape
    ...
    def selectShapePoint(self, point):
        ...
        if self.selectedVertex():  # A vertex is marked for selection.
            index, shape = 3 - self.hVertex, self.hShape
            shape.highlightVertex(index, shape.MOVE_VERTEX)
    ...
    def boundedMoveVertex(self, pos, ev):
        ...
        # help()
        point = shape[index]
        shiftPos=QPointF(0,0)
        c_index = 3 - index  
        cursor_point = shape.cursor_points[c_index]     
        if self.outOfPixmap(pos):
            pos = self.intersectionPoint(point, pos)

        if self.drawSquare:
            opposite_point_index = (index + 1) % 4
            opposite_point = shape[opposite_point_index]
            adj_point_index = index - 1
            adj_point = shape[adj_point_index]

            min_size = min(abs(pos.x() - opposite_point.x()), abs(pos.y() - opposite_point.y()))
            directionX = -1 if pos.x() - opposite_point.x() < 0 else 1
            directionY = -1 if pos.y() - opposite_point.y() < 0 else 1
            min_adsize = min(abs(pos.x() - adj_point.x()), abs(pos.y() - adj_point.y()))
            ad_directionX = -1 if pos.x() - adj_point.x() < 0 else 1
            ad_directionY = -1 if pos.y() - adj_point.y() < 0 else 1
            #if(abs(pos.x()-point.x())):
            #    shiftPos = QPointF(opposite_point.x() + directionX * min_size - point.x(),
            #                   0)
            #elif(abs(pos.y()-point.y())):
            #    shiftPos = QPointF(0,
            #                   opposite_point.y() + directionY * min_size - point.y())
            #else:
            shiftPos = QPointF(opposite_point.x() + directionX * min_size - point.x(),
                               opposite_point.y() + directionY * min_size - point.y())
            shiftadPos = QPointF(adj_point.x() + ad_directionX * min_adsize - cursor_point.x(),
                               adj_point.y() + ad_directionY * min_adsize - cursor_point.y())

        else:
            shiftPos = pos - point
            shiftadPos = pos - cursor_point

        #shape.moveVertexBy(index, shiftPos)
        #shape.moveCursorBy(c_index, shiftPos)
        # print(mods.__dict__)
        ...
        #lshift = None
        #rshift = None
        xshift = QPointF(shiftPos.x(), 0)
        yshift = QPointF(0, shiftPos.y())
        xashift = QPointF(shiftadPos.x(), 0)
        yashift = QPointF(0, shiftadPos.y())
        if index % 2 == 0:
